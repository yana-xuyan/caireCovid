...
from .modeling import (lm_loss, classification_loss, regression_loss)
from .xlnet import (XLNetConfig, create_run_config, XLNetModel)
...
def get_regression_loss(
  ...
  with tf.variable_scope("model", reuse=tf.AUTO_REUSE):
    per_example_loss, logits = regression_loss(
        hidden=summary,
        labels=label,
        initializer=xlnet_model.get_initializer(),
