...
# def get_qa_outputs_w_attention(FLAGS, features, is_training):
#   """Loss for downstream span-extraction QA tasks such as SQuAD."""
...
#   p_mask_query = tf.transpose(tf.cast(p_mask, tf.float32), [1, 0]) # size: [340, 4] type: float32
#   p_mask_inp = tf.transpose(tf.cast(p_mask, tf.int32), [1, 0])     # size: [340, 4] type: int32
#   query_toks = tf.einsum('ibh,ib->ibh', output, p_mask_query)      # size: [340, 4, 1024]
#   context_toks = tf.einsum('ibh,ib->ibh', output, 1-p_mask_query)  # size: [340, 4, 1024]

#   inp_mask_int = tf.cast(inp_mask, tf.int32)
#   query_pos = tf.cast(tf.cast(tf.math.multiply(p_mask_inp, 1-inp_mask_int), tf.bool), tf.int32)
#   context_pos = tf.cast(tf.cast(tf.math.multiply(1-p_mask_inp, 1-inp_mask_int), tf.bool), tf.int32)
#   query_len = tf.reduce_sum(tf.cast(query_pos, tf.int32), 0) - 1         # size: [340, 4]
#   context_len = tf.reduce_sum(tf.cast(context_pos, tf.int32), 0) - 1     # size: [340, 4]
...
# 	# context to query attention layer (AoA)
#   with tf.variable_scope("Context_to_Query_Attention_Layer"):
#     if is_training:
# 			# c, q, c_maxlen, q_maxlen, q_mask, c_mask
#       [max_seq_len, bsz, dimen] = output.shape.as_list()
#       c_maxlen = tf.reduce_max(context_len)
#       q_maxlen = tf.reduce_max(query_len)

#       c = tf.transpose(context_toks, [1, 0, 2])
#       c_mask = tf.transpose(context_pos, [1, 0])

#       for i in range(0, bsz):
#         q_indice0 = tf.range(context_len[i], context_len[i]+query_len[i])
#         q_indice1 = tf.zeros([query_len[i], 1])+i
#         q_indices = tf.concat([tf.reshape(q_indice0,[-1, 1]), tf.cast(q_indice1, tf.int32)], 1)
#         q_bsz = tf.gather_nd(query_toks, q_indices)
#         q_bsz = tf.concat([q_bsz, tf.zeros([q_maxlen-query_len[i], dimen])], 0)

#         q_bsz_mask = tf.gather_nd(query_pos, q_indices)
#         q_bsz_mask = tf.concat([tf.reshape(q_bsz_mask, [-1, 1]), tf.cast(tf.zeros([q_maxlen-query_len[i], 1]), tf.int32)], 0)
#         if i==0:
#           q = tf.reshape(q_bsz, [q_maxlen, 1, dimen])
#           q_mask = tf.reshape(q_bsz_mask, [q_maxlen, 1])
#         else:
#           q = tf.concat([q, tf.reshape(q_bsz, [q_maxlen, 1, dimen])], 1)
#           q_mask = tf.concat([q_mask, tf.reshape(q_bsz_mask, [q_maxlen, 1])], 1)
#       q = tf.transpose(q, [1, 0, 2])
#       q_mask = tf.transpose(q_mask, [1, 0])
		
#     else: # testing/predicting
#       [max_seq_len, _, dimen] = output.shape.as_list() # size: [340, None(?), 1024]
#       c_maxlen = tf.reduce_max(context_len)
#       q_maxlen = tf.reduce_max(query_len)
...
#       # bsz = tf.shape(output)[1]   # Tensor [340, bsz, 1024] -> Tensor [bsz]
#       # elems = tf.range(0, bsz)
#       for i in range(0, FLAGS.predict_batch_size):
#         q_indice0 = tf.range(context_len[i], context_len[i]+query_len[i])
#         q_indice1 = tf.zeros([query_len[i], 1])+i
#         q_indices = tf.concat([tf.reshape(q_indice0,[-1, 1]), tf.cast(q_indice1, tf.int32)], 1)
#         q_bsz = tf.gather_nd(query_toks, q_indices)
...
#     S = attn_utils.optimized_trilinear_for_attention([c, q], max_seq_len, q_maxlen, input_keep_prob = keep_prob) # c:[bsz, 340, dimen] q:[bsz, 64, dimen]
#     mask_q = tf.expand_dims(q_mask, 1)   # [bsz, 1, q_len]
#     S_ = tf.nn.softmax(attn_utils.mask_logits(S, mask = mask_q))  
#     mask_c = tf.expand_dims(c_mask, 2)   # [bsz, c_len, 1]
#     S_T = tf.transpose(tf.nn.softmax(attn_utils.mask_logits(S, mask = mask_c), dim = 1),(0,2,1))

#     c2q = tf.matmul(S_, q)
#     q2c = tf.matmul(tf.matmul(S_, S_T), c)
#     attention_outputs = [c, c2q, c * c2q, c * q2c]
#     output = tf.transpose(tf.concat(attention_outputs, axis = -1), [1, 0, 2])
